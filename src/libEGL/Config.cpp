...
namespace egl
{
Config::Config(D3DDISPLAYMODE displayMode, EGLint minInterval, EGLint maxInterval, D3DFORMAT renderTargetFormat, D3DFORMAT depthStencilFormat, EGLint multiSample, EGLBoolean fullscreen)
...
{
    set(displayMode, minInterval, maxInterval, renderTargetFormat, depthStencilFormat, multiSample, fullscreen);
}
...
void Config::set(D3DDISPLAYMODE displayMode, EGLint minInterval, EGLint maxInterval, D3DFORMAT renderTargetFormat, D3DFORMAT depthStencilFormat, EGLint multiSample, EGLBoolean fullscreen)
{
...
void ConfigSet::add(D3DDISPLAYMODE displayMode, EGLint minSwapInterval, EGLint maxSwapInterval, D3DFORMAT renderTargetFormat, D3DFORMAT depthStencilFormat, EGLint multiSample, EGLBoolean fullscreen)
{
    Config config(displayMode, minSwapInterval, maxSwapInterval, renderTargetFormat, depthStencilFormat, multiSample, fullscreen);
    ...
    for (Iterator config = mSet.begin(); config != mSet.end(); config++)
    {
        ...
        while (attribute[0] != EGL_NONE)
        {
            switch (attribute[0])
            {
			  ...
			  case EGL_FULLSCREEN_ANGLE:		  match = (config->mFullscreen == attribute[1]);					break;
              default:
                return false;
            }
