...
void Surface::resetSwapChain(int backbufferWidth, int backbufferHeight)
{
	...
	if(mConfig->mFullscreen) { 
		success = createFullScreen(&presentParameters); 
	} else {
		success = createWindowed(&presentParameters, backbufferWidth, backbufferHeight);
	}
    ...
    HRESULT result = device->CreateTexture(presentParameters.BackBufferWidth, presentParameters.BackBufferHeight, 1, D3DUSAGE_RENDERTARGET,
...
bool Surface::createFullScreen(D3DPRESENT_PARAMETERS* presentParameters) {
	D3DDISPLAYMODE displayMode; 
	mDisplay->getD3D9()->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &displayMode);
	IDirect3DDevice9 *device = mDisplay->getDevice();
    ...
    {        
        release();
		ERR("Could not get swap chain: %08lX", result);
        return false;
    }	
	ASSERT(SUCCEEDED(result));
...
bool Surface::createWindowed(D3DPRESENT_PARAMETERS* presentParameters, int backbufferWidth, int backbufferHeight) {
	IDirect3DDevice9 *device = mDisplay->getDevice();
	presentParameters->AutoDepthStencilFormat = mConfig->mDepthStencilFormat;
    presentParameters->BackBufferCount = 1;
    presentParameters->BackBufferFormat = mConfig->mRenderTargetFormat;
...
{    
	...
	if(mConfig->mFullscreen) {
		D3DDISPLAYMODE displayMode; 
		mDisplay->getD3D9()->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &displayMode);
		clientWidth = displayMode.Width;
		clientHeight = displayMode.Height;
	} else {
		RECT client;
