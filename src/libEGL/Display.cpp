		    ...
		    if (SUCCEEDED(result))
			{
				for (int depthStencilIndex = 0; depthStencilIndex < sizeof(depthStencilFormats) / sizeof(D3DFORMAT); depthStencilIndex++)
				{
					...
					if (SUCCEEDED(result))
					{
						...
						if (SUCCEEDED(result))
						{
							// mzechner: check whether this config supports going fullscreen, if so
							// add a duplicate config with the mFullscreen attribute set to true.
							HRESULT result = mD3d9->CheckDeviceType(mAdapter, mDeviceType, currentDisplayMode.Format, renderTargetFormat, false);

							if (SUCCEEDED(result))
							{
								configSet.add(currentDisplayMode, mMinSwapInterval, mMaxSwapInterval, renderTargetFormat, depthStencilFormat, 0, true);
							}
							
							configSet.add(currentDisplayMode, mMinSwapInterval, mMaxSwapInterval, renderTargetFormat, depthStencilFormat, 0, false);							
						}
					}
				}
			}            
...
bool Display::getConfigAttrib(EGLConfig config, EGLint attribute, EGLint *value)
{
    ...
    switch (attribute)
    {
	  ...
	  case EGL_FULLSCREEN_ANGLE:		  *value = configuration->mFullscreen;			   break;
      default:
        return false;
    }
